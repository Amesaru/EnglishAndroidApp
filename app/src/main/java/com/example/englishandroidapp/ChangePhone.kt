...
import com.example.test.databinding.ActivityChangePhoneBinding
...
class ChangePhone : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.RequestButton.setOnClickListener {
            val url = "http://localhost:8080/authApi/startPhoneConfirmation"

            val urlObj = URL(url)

            data class Body(val phone: String)

            val requestBody = Body(binding.phonePhield.text.toString())
...
//                connection.setRequestProperty("Content-Length", data.size.toString())
            ...
            if (jsonElement.isJsonObject) {
                ...
                if (success == false) {
                    showAlert("Ошибка", "Ошибка при смене телефона")
                }
        ...
        binding.confirmButton.setOnClickListener {
            val url = "http://localhost:8080/authApi/completePhoneConfirmation"
