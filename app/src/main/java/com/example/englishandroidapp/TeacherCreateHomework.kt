...
import com.example.test.databinding.ActivityTeacherCreateHomeworkBinding
...
class TeacherCreateHomework : AppCompatActivity() {

    private lateinit var binding : ActivityTeacherCreateHomeworkBinding
    private lateinit var builder : AlertDialog.Builder

    data class Body(val homework: String, val studentId: String, val topic: String)
    ...
    private fun log(Message: String) {
        Log.d("TeacherCreateHomeworkLog", Message)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityTeacherCreateHomeworkBinding.inflate(layoutInflater)
        ...
        binding.confirmButton.setOnClickListener {
            if (binding.themeField.text.toString().trim().isEmpty()) {
                showAlert("Ошибка", "Нет темы")
                return@setOnClickListener
            }

            if (binding.homeworkField.text.toString().trim().isEmpty()) {
                showAlert("Ошибка", "Введите задание")
                return@setOnClickListener
            }

            if (cur_id == "") {
                showAlert("Ошибка", "Выберите студента")
                return@setOnClickListener
            }
            log("Start")

            val requestBodyClass = Body(
                binding.homeworkField.text.toString(),
                cur_id,
            ...
            log("http://10.0.2.2:8080/homeworkTeacherApi/createHomeworkForStudent")
            val request = Request.Builder()
                .url("http://10.0.2.2:8080/homeworkTeacherApi/createHomeworkForStudent")
                .post(requestBody)
                .header("Content-Type", "application/json")
                .header("Authorization", "Bearer " + GlobalVars.accessToken)
                .build()
            ...
            client.newCall(request).enqueue(object : Callback {
                override fun onResponse(call: Call, response: Response) {
                    ...
                    if (responseCode != 200) {
                        ...
                        runOnUiThread {
                            showAlert("Успех", "Домашнее задание успешно отправлено")
                        }
                    }
                }
