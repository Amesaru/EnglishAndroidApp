...
class studentProfile : AppCompatActivity() {
    ...
    private fun log(Message: String) {
        Log.d("StudentProfileLog", Message)
    }

    private fun updateTokens() {
        log("UpdateTokens")
        val requestBodyClass = refreshBody(GlobalVars.refreshToken)
    ...
    private fun updateData() {
        ...
        val request = Request.Builder()
            .url("http://10.0.2.2:8080/profileApi/activeStep")
            .get()
            .header("Content-Type", "application/json")
    ...
    private fun updateTeacher() {
        ...
        val request = Request.Builder()
            .url("http://10.0.2.2:8080/profileApi/teacherInfo")
            .get()
            .header("Content-Type", "application/json")
            .header("Authorization", "Bearer " + GlobalVars.accessToken)
            .build()
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                if (responseBodyString != null) {
                    ...
                    if (success == false) {
                        ...
                        val data = jsonObject.getAsJsonObject("data")
                        if (data.get("fio").asString != "") {
                            runOnUiThread {
                                binding.teacherField.text = data.get("fio").asString
                            }
                        }
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.changeFIOButton.setOnClickListener {
            updateTokens()
            startActivity(Intent(this, studentChangeFio::class.java))
        }

        binding.changePhoneButton.setOnClickListener {
            updateTokens()
            startActivity(Intent(this, StudentChangePhone::class.java))
        }

        binding.changeTGButton.setOnClickListener {
            updateTokens()
            startActivity(Intent(this, StudentChangeTelegram::class.java))
        }
